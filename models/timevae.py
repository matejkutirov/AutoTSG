...
class BaseVariationalAutoencoder(nn.Module, ABC):
    ...
    def fit_on_data(self, train_data, max_epochs=1000, verbose=0):
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                loss, recon_loss, kl = self.loss_function(
            ...
            if verbose:
                print(
                    f"Epoch {epoch + 1}/{max_epochs} | Total loss: {total_loss / len(train_loader):.4f} | "
    ...
    def _get_reconstruction_loss(self, X, X_recons):
        ...
        reconst_loss += get_reconst_loss_by_axis(X, X_recons, dim=2)
        return reconst_loss
    ...
    def save_weights(self, model_dir):
        ...
        torch.save(
            ...
            os.path.join(model_dir, f"{self.model_name}_encoder_wts.pth"),
        )
        torch.save(
            self.decoder.state_dict(),
            os.path.join(model_dir, f"{self.model_name}_decoder_wts.pth"),
        )

    def load_weights(self, model_dir):
        self.encoder.load_state_dict(
            torch.load(os.path.join(model_dir, f"{self.model_name}_encoder_wts.pth"))
        )
        self.decoder.load_state_dict(
            torch.load(os.path.join(model_dir, f"{self.model_name}_decoder_wts.pth"))
        )
...
class TimeVAEEncoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim):
        ...
        self.layers.append(
            nn.Conv1d(
                feat_dim, hidden_layer_sizes[0], kernel_size=3, stride=2, padding=1
            )
        )
...
class TimeVAE(BaseVariationalAutoencoder):
    ...
    def _get_encoder(self):
        return TimeVAEEncoder(
            self.seq_len, self.feat_dim, self.hidden_layer_sizes, self.latent_dim
        )
    ...
    def save(self, model_dir: str):
        os.makedirs(model_dir, exist_ok=True)
        torch.save(
            self.state_dict(), os.path.join(model_dir, f"{self.model_name}_weights.pth")
        )
    ...
    def load(cls, model_dir: str) -> "TimeVAE":
        ...
        vae_model.load_state_dict(
            torch.load(os.path.join(model_dir, f"{self.model_name}_weights.pth"))
        )
        return vae_model
